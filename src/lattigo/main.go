...
func attack_sigmoid() {
	...
	valuesNoisy = encoder.Decode(plaintextNoisy, params.Slots())
	...
	// Re-encode
	plaintextTest := ckks.NewPlaintext(params, level, plaintextNoisy.Scale())
	encoder.Encode(plaintextTest, valuesNoisy, params.Slots()) // plaintextTest is not in NTT
...
func printPoly(r *ring.Ring, p *ring.Poly, level uint64) {
	for i := uint64(0); i < level+1; i++ {
		qi := r.Modulus[i]
		fmt.Printf("[%d] = %d %d %d %d...\n", qi, p.Coeffs[i][0], p.Coeffs[i][1], p.Coeffs[i][2], p.Coeffs[i][3])
	}
}
